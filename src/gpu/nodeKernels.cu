...
__global__ void kVelocityDistortRays(size_t rayCount, const Mat3x4f* inRays, const float* offsets, size_t offsetsCount, const Vec3f velocity, const Vec3f angularVelocity, Mat3x4f* outRays)
{
	LIMIT(rayCount);
	outRays[tid] = Mat3x4f::TRS(offsets[tid % offsetsCount] * velocity, offsets[tid % offsetsCount] * angularVelocity, Vec3f(1.0f, 1.0f, 1.0f)) * inRays[tid];
}
...
{ run(kVelocityDistortRays, stream, rayCount, inRays, offsets, offsetsCount, velocity, angularVelocity, outRays); }
