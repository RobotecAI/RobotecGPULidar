...
public:
	...
	void copyFromHost(const HostPinnedBuffer<int>& src) { copyFromHost(src.readHost(), src.getElemCount()); }

	void copyFromHost(const std::vector<T>& src) { copyFromHost(src.data(), src.size()); }

	const T* readDevice() const { return data; }

	T* writeDevice() { return data; }

	CUdeviceptr readDeviceRaw() const
	{
		return reinterpret_cast<CUdeviceptr>(reinterpret_cast<void*>(const_cast<T*>(readDevice())));
	}
