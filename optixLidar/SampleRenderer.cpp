...
/*! render one frame */
void SampleRenderer::render(std::vector<float> &lidarPoints)
{
...
void SampleRenderer::resizeLidar(uint32_t lidarSize)
{
    ...
    // update the launch parameters that we'll pass to the optix
    // launch:
    launchParams.frame.lidarSize  = lidarSize;
    launchParams.frame.lidarBuffer = (float*)lidarBuffer.d_pointer();
}
