...
protected:
	...
	GaussianNoiseAngularHitpointNodeTest() { gaussianNoiseNode = nullptr; }
};
...
{
	EXPECT_RGL_INVALID_ARGUMENT(rgl_node_gaussian_noise_angular_hitpoint(&gaussianNoiseNode, 0.0f, -1.0f, RGL_AXIS_X),
	                            "st_dev >= 0");
}

TEST_F(GaussianNoiseAngularHitpointNodeTest, invalid_argument_rotation_axis)
{
	EXPECT_RGL_INVALID_ARGUMENT(rgl_node_gaussian_noise_angular_hitpoint(&gaussianNoiseNode, 0.0f, 0.0f, (rgl_axis_t) 4),
	                            "rotation_axis");
}
...
TEST_F(GaussianNoiseAngularHitpointNodeTest, same_mean_applied_in_different_poses)
{
	...
	ASSERT_RGL_SUCCESS(rgl_node_gaussian_noise_angular_hitpoint(&gaussianNoiseNode, noiseMean, 0.0f, RGL_AXIS_Y));
