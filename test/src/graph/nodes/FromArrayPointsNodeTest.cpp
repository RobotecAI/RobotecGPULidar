...
TEST_P(FromArrayPointsNodeTest, invalid_arguments)
{
	...
	EXPECT_RGL_INVALID_ARGUMENT(
	    rgl_node_points_from_array(&usePointsNode, pointCloud->getData(), pointCloud->getPointCount(), pointFields.data(), 0),
	    "field_count > 0");

	initializeArgumentsLambda();
	EXPECT_RGL_INVALID_ARGUMENT(rgl_node_points_from_array(&usePointsNode, pointCloud->getData(), pointCloud->getPointCount(),
	                                                       nullptr, pointFields.size()),
	                            "fields != nullptr");

	initializeArgumentsLambda();
	EXPECT_RGL_INVALID_ARGUMENT(
	    rgl_node_points_from_array(&usePointsNode, pointCloud->getData(), 0, pointFields.data(), pointFields.size()),
	    "points_count > 0");

	initializeArgumentsLambda();
	EXPECT_RGL_INVALID_ARGUMENT(rgl_node_points_from_array(&usePointsNode, nullptr, pointCloud->getPointCount(),
	                                                       pointFields.data(), pointFields.size()),
	                            "points != nullptr");

	initializeArgumentsLambda();
	EXPECT_RGL_INVALID_ARGUMENT(rgl_node_points_from_array(nullptr, pointCloud->getData(), pointCloud->getPointCount(),
	                                                       pointFields.data(), pointFields.size()),
	                            "node != nullptr");
}
TEST_P(FromArrayPointsNodeTest, valid_arguments)
{
	...
	EXPECT_RGL_SUCCESS(rgl_node_points_from_array(&usePointsNode, pointCloud->getData(), pointCloud->getPointCount(),
	                                              pointFields.data(), pointFields.size()));
