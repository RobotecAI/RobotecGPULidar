...
protected:
	...
	FromMat3x4fRaysNodeTest() { raysFromMatNode = nullptr; }
};

TEST_F(FromMat3x4fRaysNodeTest, invalid_argument_node)
{
	EXPECT_RGL_INVALID_ARGUMENT(rgl_node_rays_from_mat3x4f(nullptr, &identityTestTransform, sizeof(identityTestTransform)),
	                            "node != nullptr");
}

TEST_F(FromMat3x4fRaysNodeTest, invalid_argument_rays)
{
	EXPECT_RGL_INVALID_ARGUMENT(rgl_node_rays_from_mat3x4f(&raysFromMatNode, nullptr, sizeof(identityTestTransform)),
	                            "rays != nullptr");
}
