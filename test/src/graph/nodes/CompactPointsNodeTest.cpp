...
protected:
	...
	std::pair<std::unique_ptr<TestPointCloud>, rgl_node_t> createPointCloud(
	    int pointsCount, const std::function<Field<XYZ_VEC3_F32>::type(int)>& genCoords,
	    const std::function<Field<IS_HIT_I32>::type(int)>& genIsHit)
	{
		...
		pointCloud->setFieldValues<XYZ_VEC3_F32>(pointCoordValues);
...
TEST_F(CompactPointsNodeTest, without_IS_HIT_field)
{
	...
	inPointCloud->setFieldValues<XYZ_VEC3_F32>(pointCoordValues);
