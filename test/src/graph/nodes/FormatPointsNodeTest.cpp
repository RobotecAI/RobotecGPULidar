...
class FormatPointsNodeTest : public RGLTestWithParam<int>, public FormatPointsNodeHelper { };

INSTANTIATE_TEST_SUITE_P(FormatPointsNodeTests, FormatPointsNodeTest, testing::Values(1, 100, maxGPUCoresTestCount),
...
TEST_F(FormatPointsNodeTest, invalid_argument_field_count)
{
    ...
    EXPECT_RGL_INVALID_ARGUMENT(rgl_node_points_format(&formatNodes.at(0), fields.data(), 0), "field_count > 0");
}
...
TEST_F(FormatPointsNodeTest, valid_arguments)
{
    ...
    EXPECT_RGL_SUCCESS(rgl_node_points_format(&formatNodes.at(0), fields.data(), fields.size()));
}
...
TEST_F(FormatPointsNodeTest, error_when_invalid_field_request)
{
    ...
    EXPECT_RGL_INVALID_ARGUMENT(rgl_node_points_format(&formatNodes.at(0), fields.data(), fields.size()), "cannot format field 'RGL_FIELD_DYNAMIC_FORMAT'");
}

TEST_F(FormatPointsNodeTest, format_when_empty_point_cloud_input)
{
    int pointsCount = 1;
    auto nonHitPoint = PointWithoutPadding({ 1, 2, 3 }, 0, 10);
    auto fields = PointWithoutPadding::getPointFields();

    // Simulate empty point cloud
    ASSERT_RGL_SUCCESS(rgl_node_points_from_array(&usePointsNode, &nonHitPoint, pointsCount, fields.data(), fields.size()));
    ...
    // Format empty point cloud
    ASSERT_RGL_SUCCESS(rgl_node_points_format(&formatNodes.at(0), fields.data(), fields.size()));
    ASSERT_RGL_SUCCESS(rgl_graph_node_add_child(compactPointsNode, formatNodes.at(0)));
    ...
    EXPECT_EQ(outSizeOf, sizeof(PointWithoutPadding));
    ...
    ASSERT_RGL_SUCCESS(rgl_graph_get_result_data(formatNodes.at(0), RGL_FIELD_XYZ_F32, outData.data()));
...
TEST_P(FormatPointsNodeTest, should_properly_align_output_size)
{
    ...
    auto fields = PointWithoutPadding::getPointFields();
    auto fieldsToFormat = Point4::getPointFields();
...
TEST_P(FormatPointsNodeTest, multiple_format_should_not_change_data)
{
    ...
    ASSERT_RGL_SUCCESS(rgl_graph_get_result_data(formatNodes.at(formatNodes.size() - 1), RGL_FIELD_DYNAMIC_FORMAT, outLastFormatPoints.data()));
...
TEST_P(FormatPointsNodeTest, multiple_changing_format_should_not_change_data)
{
    ...
    fieldsToFormat.push_back(Point1::getPointFields());
    fieldsToFormat.push_back(Point2::getPointFields());
    fieldsToFormat.push_back(Point3::getPointFields());
    fieldsToFormat.push_back(Point4::getPointFields());

    for (int i = 0; i < fieldsToFormat.size(); ++i) {
        ASSERT_RGL_SUCCESS(rgl_node_points_format(&formatNodes.at(i), fieldsToFormat.at(i).data(), fieldsToFormat.at(i).size()));
    }
    ...
    std::vector<Point1> outFormatPoints1(outCount);
    ...
    std::vector<Point2> outFormatPoints2(outCount);
    ...
    std::vector<Point3> outFormatPoints3(outCount);
    ...
    std::vector<Point4> outFormatPoints4(outCount);
...
TEST_F(FormatPointsNodeTest, request_for_field_that_points_do_not_have)
{
    ...
    std::vector<std::vector<rgl_field_t>> fieldsToFormat = { { XYZ_F32 }, { RAY_IDX_U32 }, { ENTITY_ID_I32 }, { INTENSITY_F32 }, { RING_ID_U16 }, { AZIMUTH_F32 }, { DISTANCE_F32 }, { RETURN_TYPE_U8 }, { TIME_STAMP_F64 } };
    ...
    auto xyzPoint = xyzAlonePoint { { 1.0f, 2.0f, 3.0f } };
