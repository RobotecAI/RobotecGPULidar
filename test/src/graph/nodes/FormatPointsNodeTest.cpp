...
TEST_F(FormatPointsNodeTests, format_node_results_data_should_be_the_same_as_from_yield_node)
{
    ...
    float deviation = 0.2;
    generateAngles(maxAngle, deviation);
...
TEST_F(FormatPointsNodeTests, multiple_format_should_not_change_data)
{
    ...
    for (int i = 0; i < outFormatPoints.size(); ++i) {
        EXPECT_EQ(outFormatPoints.at(i).xyz[0], outFormatPointsLast.at(i).xyz[0]);
        EXPECT_EQ(outFormatPoints.at(i).xyz[1], outFormatPointsLast.at(i).xyz[1]);
        EXPECT_EQ(outFormatPoints.at(i).xyz[2], outFormatPointsLast.at(i).xyz[2]);
        EXPECT_EQ(outFormatPoints.at(i).isHit, outFormatPointsLast.at(i).isHit);
        EXPECT_EQ(outFormatPoints.at(i).distance, outFormatPointsLast.at(i).distance);
    }
}
