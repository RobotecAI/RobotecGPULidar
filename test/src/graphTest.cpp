...
#define RGL_BUILD_PCL_EXTENSION 1
#if RGL_BUILD_PCL_EXTENSION
#include <rgl/api/extensions/pcl.h>

TEST_F(GraphCase, Visualize)
{
...
EXPECT_RGL_SUCCESS(rgl_node_points_compact(&compact));
EXPECT_RGL_SUCCESS(rgl_node_points_transform(&transformPts, &zeroTf));
EXPECT_RGL_SUCCESS(rgl_node_points_visualize(&visualize,  "test", 1280, 920, false));
EXPECT_RGL_SUCCESS(rgl_node_points_visualize(&visualize2,  "test2", 1280, 920, false));
...
if (i == 50) {
EXPECT_RGL_SUCCESS(rgl_node_points_visualize(&visualize3,  "test3", 1280, 920, false));
