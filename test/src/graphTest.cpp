...
TEST_F(Graph, FullLinear)
{
	...
	EXPECT_RGL_SUCCESS(rgl_node_use_rays_mat3x4f(&useRays, rays.data(), rays.size()));
	EXPECT_RGL_SUCCESS(rgl_node_transform_rays(&lidarPose, &lidarPoseTf));
	EXPECT_RGL_SUCCESS(rgl_node_raytrace(&raytrace, nullptr, 1000));
	EXPECT_RGL_SUCCESS(rgl_node_compact(&compact));
	EXPECT_RGL_SUCCESS(rgl_node_transform_points(&shear, &shearTf));
	EXPECT_RGL_SUCCESS(rgl_node_downsample(&downsample, 0.1f, 0.1f, 0.1f));
	EXPECT_RGL_SUCCESS(rgl_node_write_pcd_file(&write, "minimal.pcd"));
...
TEST_F(Graph, NodeActivation)
{
	...
	EXPECT_RGL_SUCCESS(rgl_node_raytrace(&raytrace, nullptr, 1000));
	EXPECT_RGL_SUCCESS(rgl_node_compact(&compact));
	EXPECT_RGL_SUCCESS(rgl_node_transform_points(&transformPts, &zeroTf));
	EXPECT_RGL_SUCCESS(rgl_node_write_pcd_file(&write, "two_boxes_activation.pcd"));
	...
	EXPECT_RGL_SUCCESS(rgl_node_transform_points(&transformPts, &translateXTf));
	EXPECT_RGL_SUCCESS(rgl_graph_node_set_active(transformPts, false));
	EXPECT_RGL_SUCCESS(rgl_graph_run(write));

	// Activate transform points
	EXPECT_RGL_SUCCESS(rgl_node_transform_points(&transformPts, &translateYTf));
...
TEST_F(Graph, NodeRemoval)
{
	...
	EXPECT_RGL_SUCCESS(rgl_node_compact(&compact));
	EXPECT_RGL_SUCCESS(rgl_node_transform_points(&transformPts, &zeroTf));
	EXPECT_RGL_SUCCESS(rgl_node_write_pcd_file(&write, "two_boxes_removal.pcd"));
...
TEST_F(Graph, FormatNodeResults)
{
	...
	EXPECT_RGL_SUCCESS(rgl_node_transform_rays(&lidarPose, &lidarPoseTf));
	EXPECT_RGL_SUCCESS(rgl_node_raytrace(&raytrace, nullptr, 1000));
	EXPECT_RGL_SUCCESS(rgl_node_format(&format, formatFields.data(), formatFields.size()));
