...
protected:
	...
	float toRadians(float angle_radians) { return angle_radians * M_PI / 180; }
...
TEST_F(Transforms, RayMoveZGaussianNoise)
{
	...
	EXPECT_RGL_SUCCESS(rgl_lidar_set_gaussian_noise_params(lidar, rgl_angular_noise_type_t::RGL_ANGULAR_NOISE_TYPE_RAY_BASED,
	                                                       0.0, 0.0, distance_std_dev, 0.0, 0.0));
...
TEST_F(Transforms, RayMoveYGaussianNoise)
{
	for (auto ray_move_distance_y : distances_out_entity) {
		...
		rgl_lidar_destroy(lidar);
		rgl_lidar_create(&lidar, tf_rays_moved.data(), tf_rays_moved.size());
		EXPECT_RGL_SUCCESS(rgl_lidar_set_gaussian_noise_params(
		    lidar, rgl_angular_noise_type_t::RGL_ANGULAR_NOISE_TYPE_RAY_BASED, 0.0, 0.0, distance_std_dev, 0.0, 0.0));
